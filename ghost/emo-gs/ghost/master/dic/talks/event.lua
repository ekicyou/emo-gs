...
function EV:初回起動(data, req)
    local s = [=[\1\![move,-353,,,0,base,base]\s[素]]=]
    s = s.. [=[\0\c\s[笑顔]はじめましてや！！\nうちは、むらさき。]=]
    s = s.. [=[\1\c\s[素]僕はエモ。\s[笑顔]クール系の可愛い娘。]=]
    s = s.. [=[\0\n[150]\s[へえ]そこ自分でいう‥‥。]=]
    s = s.. [=[\1\c\s[照れ怒り]イイジャン！‥‥ええと、\0\s[素]\1\s[静観]僕は日常から「感情」を探してるんだ。]=]
    s = s.. [=[\0\c\s[静観]つまり、、\n\s[いたずら笑顔]エモを弄ってればOK？]=]
...
function EV:起動(data, req)
    local s = [=[\1\s[10]\0\s[笑顔]起動したで！\1\s[素]や、お久しぶり。\e]=]
    return response.talk(s)
end

--クリック
function EV:ダブルクリック(data, req)
    local s = [=[\1\![move,-353,,,0,base,base]\s[100]\0\s[素]ダブルクリックで位置調整なんや。\1\s[100]ソーシャルディスタンスだね。\e]=]
    return response.talk(s)
end
...
function EV:バルーン切り替え(data, req)
    local s = [=[\1\s[素]\0\s[素]バルーン切り替えです。\w9\w9\1\s[素]ばるるるるん？\_w[600]\e]=]
    return response.ok(s)
end

--ネットワーク更新：ネットワーク更新開始
function EV:更新開始(data, req)
    local s = [=[\1\s[素]\0\s[笑顔]あったらし～い～、更新は？\1\s[素]きた？\e]=]
    return response.talk(s)
end
--ネットワーク更新：更新ファイル確認
function EV:更新確認(data, req)
    local s = [=[\1\s[素]\0\s[期待大]きたよぉ～。\1\s[冷笑]期待しない方が‥‥\e]=]
    return response.talk(s)
end
--ネットワーク更新：更新されなかった
function EV:更新無し(data, req)
    local s = [=[\1\s[素]\0\s[期待外れ]こえへん‥‥\w9\w9\1\s[驚き]ｲｷﾛｰ｡\e]=]
    return response.talk(s)
end
--ネットワーク更新：サイト無し
function EV:更新失敗(data, req)
    local s = [=[\1\s[素]\0\s[期待外れ]繋がらへん‥‥\w9\w9\1\s[驚き]ｲｷﾛｰ｡\e]=]
    return response.talk(s)
end
--ネットワーク更新：更新された
